...
app.layout = html.Div(
    [
        ...
        dcc.Dropdown(
            ...
            options=[{"label": prop, "value": prop} for prop in ["style_cell", "data"]],
        ),
        html.Br(),
        dash_table.DataTable(
            ...
            data=[
                {"column-{}".format(i): (randint(0, 100)) for i in range(1, 5)}
                for j in range(5)
            ],
            editable=True,
        ),
...
def loading_data(value):
    if value == "data":
        sleep(5)
        return [
            {"column-{}".format(i): (randint(0, 100)) for i in range(5)}
            for j in range(5)
        ]
    raise PreventUpdate
